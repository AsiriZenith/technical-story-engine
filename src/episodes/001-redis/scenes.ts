...
export const redisScenes = [
  ...
  },
  {
    ...
    visualNotes: [
      'Reuses the exact approved Scene 07 technical-architecture PNG as the visual foundation; never redrawn or recomposed as native Remotion nodes.',
      'A single Redis SpotlightImage target is held across the prepared-result / simple-lookup / in-memory beats, then released (not handed to Database) for the avoided-work payoff beat -- a deliberate departure from Scenes 08/09 node-to-node handoffs, since this scene explains why one step is fast rather than retracing the request path.',
      'Database is never spotlighted in this scene.',
    ],
  },
  {
    ...
    durationStrategy: sharedWindow(9, 2),
    assetIds: [],
    storyBeats: [
      'So... why not avoid repeated work everywhere?',
      'CACHE EVERYTHING! -> Not so fast.',
    ...
    visualNotes: [
      'Fully Remotion-native, typography-driven scene -- no image asset, no SpotlightImage. Scene 01 is the visual precedent: centered Interactive.Div text blocks, typography tokens, theme colors, and the existing motionPresets/interpolate() vocabulary.',
      'The only humorous beat ("CACHE EVERYTHING!" -> "Not so fast.") is purely typographic: no character illustration, no new asset. The formerly-listed developer placeholder asset is intentionally not used.',
      'Costs (memory / stale data / invalidation) and the "not every request qualifies" point are each a single plain sentence, not cards/chips/badges, to avoid a checklist-slide feel.',
    ],
  },
  {
    id: '12-stale-cache',
    ...
    storyBeats: [
      ...
      'Database: alice.new@example.com (current value changes)',
      'Redis: alice.old@example.com (cached copy stays old)',
      'The application reads Redis -- and returns that answer fast',
      'Wrong answer. Very fast.',
      'Caching reuses old work. That is also how it can become stale.',
    ],
    visualNotes: [
      'Fully Remotion-native, typography-driven scene -- no image asset, no SpotlightImage. Continues Scene 11\'s Option D visual language with a second concrete use case: a two-card Database-vs-Redis value comparison, both cards held on screen together so the divergence is simultaneous, not just narrated.',
      'The Alice email example (alice.new@example.com / alice.old@example.com) already established in the outline is used unchanged. Values are not translated.',
      'The only humorous beat ("Wrong answer. Very fast.") is purely typographic, calm and wry, not a large comedic spectacle. The formerly-listed alicePortrait placeholder asset is intentionally not used.',
    ],
  },
  {
    ...
    visualNotes: [
      'Fully Remotion-native, hybrid central-node scene -- no image asset, no SpotlightImage. A small central Redis node (Scene 01\'s cache-node accent/stack-icon vocabulary, simplified) stays on screen from Beat 2 onward; one of four callouts (Freshness / Capacity / Resilience / Operations) is fully active at a time around it, each paired with a small ease-out pulse on the central node, never an arrow or connecting line.',
      'Grouping replaces the earlier overlapping "Memory and cache misses" / "Monitoring and failure handling" wording with four non-overlapping categories: Freshness (expiration+invalidation), Capacity (memory), Resilience (misses+failures), Operations (monitoring+infrastructure).',
      'No humor in this scene, per the approved plan -- tone stays calm and matter-of-fact.',
    ],
  },
  {
